...
const PRESERVE_HEADER = Buffer.from('%BYTENODE-PRESERVE%');
const sheBangRegex = /^#!.*/;
...
function generateScript (cachedData, filename) {
  ...
  const preserveIndex = cachedData.indexOf(PRESERVE_HEADER);
  if (preserveIndex !== -1) {
    const ranges = JSON.parse(
      cachedData.subarray(preserveIndex + PRESERVE_HEADER.length).toString()
    );
    for (const [start, end, code] of ranges) {
      dummyCode = dummyCode.slice(0, start) + code + dummyCode.slice(end);
    }
  }
...
const compileCode = function (javascriptCode, compress) {
  ...
  bytecodeBuffer = Buffer.concat([bytecodeBuffer, encodeFunctionSources(javascriptCode)]);
...
function encodeFunctionSources (javascriptCode) {
  const REGEX = /\/[/*]\s*bytenode-preserve-start\s*(.+?)\/[/*]\s*bytenode-preserve-end/gsd;
  const ranges = Array.from(javascriptCode.matchAll(REGEX),
    match => [...match.indices[1], match[1]]
  );
  if (ranges.length === 0) return Buffer.alloc(0);

  const encodedRanges = Buffer.from(JSON.stringify(ranges));
  return Buffer.concat([PRESERVE_HEADER, encodedRanges]);
}
