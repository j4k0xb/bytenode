...
const compileCode = function (javascriptCode, compress) {
  ...
  if (javascriptCode.includes('bytenode-preserve')) {
    const ast = parser.parse(javascriptCode);

    const statementTemplate = template`ID.toString = () => "SRC";`;
    const expressionTemplate = template`
      (fn => {
        fn.toString = () => "SRC";
        return fn;
      })(FN)`;
    ...
    traverse(ast, {
      ...
      Function(path) {
        ...
        if (
          ...
          node.leadingComments[0].value.trim() === "bytenode-preserve"
        ) {
          const src = javascriptCode.slice(node.loc.start.index, node.loc.end.index);

          if (path.isStatement()) {
            path.insertAfter(statementTemplate({ ID: node.id, SRC: src }));
          } else {
            path.replaceWith(expressionTemplate({ FN: node, SRC: src }));
          }
        }
      },
    });

    javascriptCode = generate(ast).code;
  }
